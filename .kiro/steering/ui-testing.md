...
## MANDATORY UI Testing Requirements

**КРИТИЧЕСКИ ВАЖНО:** При работе с любыми UI компонентами ОБЯЗАТЕЛЬНО создавать UI тесты.

### Обязательные UI тесты для всех интерфейсных изменений:

1. **При создании новых UI компонентов:**
   - [ ] Тесты инициализации компонента
   - [ ] Тесты всех интерактивных элементов (кнопки, поля ввода, выпадающие списки)
   - [ ] Тесты обработки событий (клики, изменения значений)
   - [ ] Тесты состояний компонента (активный, неактивный, загрузка)

2. **При работе с модальными окнами:**
   - [ ] Тесты открытия модального окна
   - [ ] Тесты содержимого модального окна
   - [ ] Тесты закрытия модального окна (кнопка, ESC, клик вне окна)
   - [ ] Тесты валидации форм в модальном окне
   - [ ] Тесты обработки ошибок при отсутствии page объекта

3. **При работе с кнопками:**
   - [ ] Тесты атрибутов кнопки (текст, иконка, цвет, размер)
   - [ ] Тесты callback функций при нажатии
   - [ ] Тесты состояний кнопки (активная, неактивная, загрузка)
   - [ ] Тесты tooltip и accessibility

4. **При работе с формами:**
   - [ ] Тесты валидации полей
   - [ ] Тесты отображения ошибок валидации
   - [ ] Тесты сохранения данных
   - [ ] Тесты очистки формы

5. **При работе с адаптивными компонентами:**
   - [ ] Property-based тесты с различными размерами экрана
   - [ ] Тесты переключения между режимами отображения
   - [ ] Тесты стабильности при изменении размеров

### Правило "Нет UI изменений без UI тестов"

**ЗАПРЕЩЕНО:**
- Создавать или изменять UI компоненты без соответствующих тестов
- Коммитить изменения интерфейса без покрытия тестами
- Исправлять UI баги без добавления регрессионных тестов

**ОБЯЗАТЕЛЬНО:**
- Каждый новый UI компонент должен иметь тестовый файл `test_*_ui.py`
- Каждое изменение существующего UI должно обновлять соответствующие тесты
- Все интерактивные элементы должны быть покрыты тестами

### Checklist перед коммитом UI изменений:

- [ ] **Использован современный Flet API (проверен через документацию)**
- [ ] Созданы unit тесты для всех новых UI компонентов
- [ ] Созданы property-based тесты для адаптивных элементов
- [ ] Протестированы все интерактивные элементы (кнопки, формы, модальные окна)
- [ ] Добавлены тесты обработки ошибок
- [ ] Проверена работа без page объекта
- [ ] Все тесты проходят успешно
- [ ] Покрытие кода UI компонентов > 80%
...
6. **Обязательность** - UI изменения без тестов недопустимы
7. **Актуальность API** - ВСЕГДА использовать современную версию Flet API
...
## ABSOLUTE PROHIBITION: Performance Tests

**КРИТИЧЕСКИ ВАЖНО - АБСОЛЮТНЫЙ ЗАПРЕТ:**

### ❌ ЗАПРЕЩЕНО создавать тесты производительности

**В этом проекте ЗАПРЕЩЕНЫ любые тесты производительности, включая:**
- Тесты с измерением времени выполнения (`time.time()`, `time.perf_counter()`)
- Stress-тесты с множественными итерациями для проверки производительности
- Тесты с проверками типа `assertLess(execution_time, max_time)`
- Любые тесты, которые проверяют скорость выполнения операций
- Тесты с названиями содержащими "performance", "stress", "speed", "benchmark"

**Что РАЗРЕШЕНО тестировать:**
- ✅ Функциональность и логику приложения
- ✅ UI компоненты и их поведение
...
**Если нужно проверить работу с большими объёмами данных:**
- Создавайте тесты, которые проверяют КОРРЕКТНОСТЬ обработки, а не скорость
- Проверяйте, что система не падает и возвращает правильные результаты
- НЕ измеряйте время выполнения
...
11. **❌ НИКОГДА не создавайте тесты производительности** - см. раздел "ABSOLUTE PROHIBITION"
