...
│   └── specs/
├── concepts/                      # Концепты и идеи развития (не в Git)
│   ├── features/                  # Идеи новых функций
...
# Пользовательские данные (рядом с приложением)
...
class Config:
    ...
    def get_user_data_dir() -> Path:
        ...
        Returns:
            Path: .finance_tracker_data/ относительно корня приложения
        """
        ...
        if getattr(sys, 'frozen', False):
            ...
            # Режим разработки - используем корень проекта
...
def get_database_path() -> str:
    ...
    Returns:
        str: Путь к finance.db в .finance_tracker_data/ относительно приложения
    """
...
*For any* файл пользовательских данных (база данных, логи, настройки), он должен находиться в директории `.finance_tracker_data/` относительно корня приложения (рядом с .exe в режиме .exe, в корне проекта в режиме разработки), а не разбросаны по корню проекта.
...
3. **Property Test: Пользовательские данные изолированы**
   ...
   - Проверить, что БД создаётся в .finance_tracker_data/ относительно приложения
   - Проверить, что логи создаются в .finance_tracker_data/logs/
   - Проверить, что пользовательские данные не разбросаны по корню проекта
...
6. **Обновление путей к данным**
   - Обновить config.py для использования .finance_tracker_data/ относительно приложения
