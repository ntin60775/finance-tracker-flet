...
get_all_categories = get_selectable_leaf_categories
...
class TransactionModal:
    ...
    def __init__(
        ...
        self.amount_field = ft.TextField(
            ...
            input_filter=ft.InputFilter(
                allow=True,
                regex_string=r"^-?(\d+\.?\d{0,2}|0\.?\d{0,2}|\.\d{1,2})$",
                replacement_string="",
            ),
        ...
        self.category_dropdown = ft.Dropdown(
            label="Категория", options=[], on_select=self._on_category_change
        )
    ...
    def open_edit(self, page: ft.Page, transaction: TransactionDB):
        ...
        try:
            logger.debug(
                f"Открытие TransactionModal в режиме редактирования для транзакции: {transaction.id}"
            )
            ...
            logger.error(
                f"Ошибка при открытии TransactionModal в режиме редактирования: {e}", exc_info=True
            )
        ...
        try:
            ...
            if self.edit_mode and self.editing_transaction:
                ...
                if str(self.editing_transaction.category_id) in category_ids:
                    ...
                    logger.debug(
                        f"Установлена категория из редактируемой транзакции: {self.editing_transaction.category_id}"
                    )
                else:
                    self.category_dropdown.value = None
                    logger.warning(
                        f"Категория {self.editing_transaction.category_id} не найдена среди категорий типа {t_type}"
                    )
            else:
    ...
    def _save(self, e):
        ...
        try:
            logger.debug(
                f"Сохранение транзакции в режиме: {'редактирование' if self.edit_mode else 'создание'}"
            )
            ...
            if self.edit_mode:
                ...
                logger.debug(
                    f"Создан объект TransactionUpdate для транзакции {self.editing_transaction.id}"
                )
