...
def _calculate_day_totals(transactions):
    total_income = sum(
        transaction.amount
...
def _transaction_amount_meta(transaction_type):
    if transaction_type == TransactionType.INCOME:
        return ft.Colors.GREEN, "+"
    return ft.Colors.RED, "-"


def _loan_payment_status_meta(status):
    executed_with_delay_status = getattr(PaymentStatus, "EXECUTED_WITH_DELAY")

    status_colors = {
        PaymentStatus.PENDING: ft.Colors.BLUE,
        PaymentStatus.EXECUTED: ft.Colors.GREEN,
        executed_with_delay_status: ft.Colors.ORANGE,
    ...
    status_icons = {
        ...
        executed_with_delay_status: ft.Icons.WARNING,
    ...
    status_labels = {
        ...
        executed_with_delay_status: "Выполнен с просрочкой",
        PaymentStatus.OVERDUE: "Просрочен",
        PaymentStatus.CANCELLED: "Отменён",
    }

    icon_color = status_colors.get(status, ft.Colors.BLUE)
    icon = status_icons.get(status, ft.Icons.PAYMENT)
    status_label = status_labels.get(status, status.value)
    ...
    elif status == PaymentStatus.EXECUTED:
        bgcolor = ft.Colors.GREEN_50

    return icon_color, icon, status_label, bgcolor


def _build_loan_payment_subtitle(payment: LoanPaymentDB, status_label: str) -> str:
    subtitle_parts = [
...
class TransactionsPanel(ft.Container):
    ...
    def __init__(
        ...
        self.pending_payments_list = ft.ListView(
            spacing=10, padding=10
    ...
    def _update_view(self):
        ...
        planned_title = next(
            (
                ...
                if isinstance(c, ft.Text) and c.key == "planned_title"
            ),
            None,
        )
        ...
        pending_title = next(
            (
                c
                for c in self.content.controls
                if isinstance(c, ft.Text) and c.key == "pending_title"
            ),
            None,
        )
        ...
        if not self.pending_payments:
            ...
            for payment in self.pending_payments:
                self.pending_payments_list.controls.append(
                    self._build_pending_payment_tile(payment)
                )

        # 5.2. Обновляем список платежей по кредитам
        self.loan_payments_list.controls.clear()
        loan_payments_title = next(
            (
                c
                for c in self.content.controls
                if isinstance(c, ft.Text) and c.key == "loan_payments_title"
            ),
            None,
        )
    ...
    def _build_transaction_tile(self, transaction: TransactionDB) -> ft.ListTile:
        ...
        amount_color, amount_sign = _transaction_amount_meta(transaction.type)
        ...
        return ft.Container(
            content=ft.ListTile(
                leading=ft.Icon(
                    ...
                    if transaction.type == TransactionType.INCOME
                    else ft.Icons.ARROW_CIRCLE_DOWN,
                    color=amount_color,
                ),
    ...
    def _safe_edit_transaction(self, transaction: TransactionDB):
        ...
        try:
            ...
            logger.error(
                f"Ошибка при вызове callback редактирования транзакции: {ex}", exc_info=True
            )
    ...
    def _build_occurrence_tile(self, occurrence: PlannedOccurrence) -> ft.ListTile:
        ...
        try:
            ...
            logger.warning(
                f"Could not resolve planned transaction details for occurrence {occurrence.id}: {e}"
            )
        ...
        if occurrence.status == OccurrenceStatus.PENDING:
            trailing = ft.Row(
                controls=[
                    ft.IconButton(
                        ...
                        tooltip="Исполнить",
                        on_click=lambda _, o=occurrence: (
                            self.on_execute_occurrence(o) if self.on_execute_occurrence else None
                        ),
                        ...
                        style=ft.ButtonStyle(
                            ...
                            bgcolor={
                                ...
                                if self.on_execute_occurrence is not None
                    ...
                    ft.IconButton(
                        ...
                        tooltip="Пропустить",
                        on_click=lambda _, o=occurrence: (
                            self.on_skip_occurrence(o) if self.on_skip_occurrence else None
                        ),
                        ...
                        style=ft.ButtonStyle(
                            ...
                            bgcolor={
                                ...
                                if self.on_skip_occurrence is not None
                                else ft.Colors.TRANSPARENT,
                            },
                            icon_color={
            ...
            status_text = (
                "Исполнено" if occurrence.status == OccurrenceStatus.EXECUTED else "Пропущено"
            )
    ...
    def _build_pending_payment_tile(self, payment: PendingPaymentDB) -> ft.ListTile:
        ...
        trailing = ft.IconButton(
            ...
            on_click=lambda _, p=payment: (
                self.on_execute_pending_payment(p) if self.on_execute_pending_payment else None
            ),
            ...
            style=ft.ButtonStyle(
                ...
                bgcolor={
                    ft.ControlState.DEFAULT: ft.Colors.TRANSPARENT,
                    ft.ControlState.HOVERED: ft.Colors.GREEN_50
                    if self.on_execute_pending_payment is not None
                    else ft.Colors.TRANSPARENT,
                },
                icon_color={
                    ft.ControlState.DEFAULT: ft.Colors.GREEN
        ...
        return ft.ListTile(
            ...
            subtitle=ft.Text(
                f"{payment.description} • {payment.amount:,.2f} ₽ • {payment.priority.value}"
            ),
            trailing=trailing,
            bgcolor=ft.Colors.BLUE_50,
        )
    ...
    def _build_loan_payment_tile(self, payment: LoanPaymentDB) -> ft.ListTile:
        ...
        icon_color, icon, status_label, bgcolor = _loan_payment_status_meta(payment.status)
        subtitle = _build_loan_payment_subtitle(payment, status_label)
        ...
        if payment.status in (PaymentStatus.PENDING, PaymentStatus.OVERDUE):
            trailing = ft.IconButton(
                ...
                tooltip="Исполнить платёж",
                on_click=lambda _, p=payment: (
                    self.on_execute_loan_payment(p) if self.on_execute_loan_payment else None
                ),
                ...
                style=ft.ButtonStyle(
                    ...
                    bgcolor={
                        ft.ControlState.DEFAULT: ft.Colors.TRANSPARENT,
                        ft.ControlState.HOVERED: ft.Colors.GREEN_50
                        if self.on_execute_loan_payment is not None
                        else ft.Colors.TRANSPARENT,
                    },
                    icon_color={
                        ft.ControlState.DEFAULT: ft.Colors.GREEN
