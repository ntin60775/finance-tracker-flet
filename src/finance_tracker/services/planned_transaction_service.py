...
def create_planned_transaction(
    ...
    try:
        ...
        occurrence_dates = generate_occurrences_for_period(
            session, new_planned_tx, period_start, period_end
        )
        ...
        for scheduled_date in occurrence_dates:
            ...
            existing_occurrence = (
                session.query(PlannedOccurrenceDB)
                .filter_by(planned_transaction_id=new_planned_tx.id, occurrence_date=scheduled_date)
                .first()
            )
...
def update_planned_transaction(
    session: Session, planned_tx_id: str, planned_tx: PlannedTransactionCreate
    ...
    try:
        ...
        if planned_tx.recurrence_rule and existing_tx.recurrence_rule:
            ...
            existing_tx.recurrence_rule.end_condition_type = (
                planned_tx.recurrence_rule.end_condition_type
            )
            existing_tx.recurrence_rule.end_date = planned_tx.recurrence_rule.end_date
            existing_tx.recurrence_rule.occurrences_count = (
                planned_tx.recurrence_rule.occurrences_count
            )
        ...
        logger.info(
            f"Обновлена плановая транзакция ID {planned_tx_id}, новая сумма {planned_tx.amount}"
        )
...
def deactivate_planned_transaction(session: Session, planned_tx_id: str) -> bool:
    """
...
def delete_planned_transaction(
    session: Session, planned_tx_id: str, delete_actual_transactions: bool = False
) -> bool:
    """
    ...
    try:
        ...
        occurrences = (
            session.query(PlannedOccurrenceDB).filter_by(planned_transaction_id=planned_tx_id).all()
        )
        ...
        actual_transactions = (
            ...
            .filter(TransactionDB.planned_occurrence_id.in_(occurrence_ids))
            .all()
        )
        ...
        for actual_tx in actual_transactions:
            if delete_actual_transactions:
                ...
                logger.debug(
                    f"Очищена ссылка на плановое вхождение для транзакции ID {actual_tx.id}"
                )
...
def get_all_planned_transactions(
    session: Session, active_only: bool = True, transaction_type: Optional[TransactionType] = None
...
def get_occurrences_by_date(session: Session, occurrence_date: date) -> List[PlannedOccurrenceDB]:
    """
    ...
    try:
        occurrences = (
            session.query(PlannedOccurrenceDB).filter_by(occurrence_date=occurrence_date).all()
        )
        return occurrences
    except SQLAlchemyError as e:
...
def get_pending_occurrences(session: Session, limit: int = 5) -> List[PlannedOccurrenceDB]:
    """
    ...
    try:
        ...
        priority = case((PlannedOccurrenceDB.occurrence_date < today, 0), else_=1)

        occurrences = (
            session.query(PlannedOccurrenceDB)
            .filter(PlannedOccurrenceDB.status == OccurrenceStatus.PENDING)
            .order_by(
...
def execute_occurrence(
    session: Session, occurrence_id: str, execution_date: date, amount: Decimal
) -> TransactionDB:
    """
...
def skip_occurrence(session: Session, occurrence_id: str) -> PlannedOccurrenceDB:
    """
...
def reschedule_occurrence(
    session: Session, occurrence_id: str, new_date: date
) -> PlannedOccurrenceDB:
    """
    ...
    if occurrence.status != OccurrenceStatus.PENDING:
        raise ValueError(
            f"Можно переносить только ожидающие вхождения (текущий статус: {occurrence.status.value})"
        )
