...
class TestCalendarLegendWidthFixIntegration(unittest.TestCase):
    ...
    def test_performance_with_frequent_resizing(self):
        """
        Тест производительности при частом изменении размеров.
        
        Проверяет:
        - Отсутствие утечек памяти при частых изменениях
        - Приемлемое время отклика
        - Стабильность при интенсивном использовании
        """
        import time
        ...
        with patch('finance_tracker.database.get_db_session') as mock_get_db:
            ...
            # Act - симулируем частое изменение размеров
            start_time = time.time()
            
            # Множественные изменения размера
            for i in range(100):
                width = 300 + (i % 500)  # Ширина от 300 до 800
                legend.update_calendar_width(width)
                
                # Строим UI для каждого размера
                if legend._should_show_full_legend():
                    content = legend._build_full_legend()
                else:
                    content = legend._build_compact_legend()
                
                # Проверяем стабильность каждые 10 итераций
                if i % 10 == 0:
                    self.assertIsInstance(content, ft.Row,
                                        f"UI должен оставаться стабильным на итерации {i}")
                    self.assertGreater(len(content.controls), 0,
                                     f"UI должен содержать элементы на итерации {i}")
            ...
            # 1. Общее время должно быть приемлемым (менее 5 секунд для 100 изменений)
            self.assertLess(total_time, 5.0,
                          f"100 изменений размера заняли слишком много времени: {total_time:.2f}с")
            
            # 2. Среднее время на одно изменение должно быть быстрым
            avg_time = total_time / 100
            self.assertLess(avg_time, 0.05,
                          f"Среднее время на изменение размера слишком велико: {avg_time:.3f}с")
            
            # 3. Система должна оставаться функциональной после интенсивного использования
            final_width = 600
            legend.update_calendar_width(final_width)
            
            try:
                final_content = legend._build_full_legend() if legend._should_show_full_legend() else legend._build_compact_legend()
                final_modal = legend.modal_manager.create_modal(legend.all_indicators)
                
                self.assertIsInstance(final_content, ft.Row,
                                    "UI должен оставаться функциональным после интенсивного использования")
                self.assertIsInstance(final_modal, ft.AlertDialog,
                                    "Модальное окно должно создаваться после интенсивного использования")
                
            except Exception as e:
                self.fail(f"Система должна оставаться функциональной после интенсивного использования: {e}")
