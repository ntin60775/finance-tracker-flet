...
from finance_tracker.views.loan_details_view import (
    LoanDetailsView,
    _filter_payments_for_tab,
    _format_currency,
    _transfer_difference_meta,
)
...
class TestLoanDetailsView(ViewTestBase):
    ...
    def setUp(self):
        ...
        self.mock_get_db = self.add_patcher(
            "finance_tracker.views.loan_details_view.get_db_session", return_value=self.mock_db_cm
        )

        # Патчим сервисы кредитов
        self.mock_get_loan_by_id = self.add_patcher(
            "finance_tracker.views.loan_details_view.get_loan_by_id", return_value=None
        )
        self.mock_get_lender_by_id = self.add_patcher(
            "finance_tracker.views.loan_details_view.get_lender_by_id", return_value=None
        )

        # Патчим сервисы платежей
        self.mock_get_payments_by_loan = self.add_patcher(
            "finance_tracker.views.loan_details_view.get_payments_by_loan", return_value=[]
        )
        ...
        self.mock_get_transfer_history = self.add_patcher(
            "finance_tracker.views.loan_details_view.get_transfer_history", return_value=[]
        )
    ...
    def test_load_loan_details_success(self):
        ...
        self.assert_service_called(self.mock_get_loan_by_id, self.mock_session, 1)
        self.assert_service_called(self.mock_get_lender_by_id, self.mock_session, 1)
    ...
    def test_execute_payment_action(self):
        ...
        test_payment = create_test_loan_payment(id=1, loan_id=1, status=PaymentStatus.PENDING)
    ...
    def test_handle_early_repayment_full(self):
        ...
        self.view.handle_early_repayment(
            is_full=True, amount=Decimal("100000.00"), repayment_date=date.today()
        )

        # Проверяем, что early_repayment_full был вызван
        self.assert_service_called(
            self.mock_early_repayment_full, self.mock_session, 1, Decimal("100000.00"), date.today()
        )
    ...
    def test_handle_early_repayment_partial(self):
        ...
        # Вызываем handle_early_repayment
        self.view.handle_early_repayment(
            is_full=False, amount=Decimal("50000.00"), repayment_date=date.today()
        )
    ...
    def test_load_transfer_history_with_transfers(self):
        ...
        self.assert_service_called(self.mock_get_transfer_history, self.mock_session, 1)
    ...
    def test_filter_payments_for_tab_returns_expected_statuses(self):
        payments = [
            create_test_loan_payment(status=PaymentStatus.PENDING),
            create_test_loan_payment(status=PaymentStatus.OVERDUE),
            create_test_loan_payment(status=PaymentStatus.EXECUTED),
            create_test_loan_payment(status=PaymentStatus.EXECUTED_LATE),
        ]

        schedule_payments = _filter_payments_for_tab(payments, selected_index=0)
        history_payments = _filter_payments_for_tab(payments, selected_index=1)

        self.assertEqual(len(schedule_payments), 2)
        self.assertTrue(
            all(
        ...
        self.assertEqual(len(history_payments), 2)
        self.assertTrue(
            all(
                payment.status in (PaymentStatus.EXECUTED, PaymentStatus.EXECUTED_LATE)
                for payment in history_payments
            )
        )

    def test_transfer_difference_meta_and_currency_formatting(self):
        positive_text, positive_color, positive_icon = _transfer_difference_meta(Decimal("1500.50"))
        negative_text, negative_color, negative_icon = _transfer_difference_meta(Decimal("-20.00"))
        zero_text, zero_color, zero_icon = _transfer_difference_meta(Decimal("0"))

        import flet as ft

        self.assertEqual(positive_text, "+1 500.50 ₽")
        self.assertEqual(positive_color, ft.Colors.RED)
        self.assertEqual(positive_icon, ft.Icons.ARROW_UPWARD)

        self.assertEqual(negative_text, "-20.00 ₽")
        self.assertEqual(negative_color, ft.Colors.GREEN)
        self.assertEqual(negative_icon, ft.Icons.ARROW_DOWNWARD)

        self.assertEqual(zero_text, "0.00 ₽")
        self.assertEqual(zero_color, ft.Colors.GREY)
        self.assertEqual(zero_icon, ft.Icons.REMOVE)
        self.assertEqual(_format_currency(Decimal("1234.56")), "1 234.56 ₽")
