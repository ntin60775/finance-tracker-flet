...
class TestTransactionModal(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.patcher = patch(
            "finance_tracker.components.transaction_modal.get_selectable_leaf_categories"
        )
        ...
        self.modal = TransactionModal(session=self.session, on_save=self.on_save)
        self.page = MagicMock()
        self.page.open = Mock()
    ...
    def test_open_modal(self):
        ...
        self.assertEqual(self.modal.category_dropdown.options[0].text, "Food / Cafe")
    ...
    def test_leaf_expense_categories_are_used_in_dropdown(self):
        self.modal.open(self.page)
    ...
    def test_leaf_category_label_includes_parent_path(self):
        self.modal.open(self.page)
    ...
    def test_transaction_database_save_and_modal_close(self):
        ...
        self.modal.category_dropdown.value = (
            self.cat_id_1
        )  # Используем существующую категорию "Food"
        ...
        self.assertTrue(
            self.modal.amount_field.error_text == "" or self.modal.amount_field.error_text is None,
            "Не должно быть ошибок валидации для поля суммы",
        )
        self.assertTrue(
            self.modal.category_dropdown.error_text == ""
            or self.modal.category_dropdown.error_text is None,
        ...
        self.assertIsNotNone(
            called_transaction_data.category_id, "ID категории не должен быть None"
        )
    ...
    def test_transaction_save_with_income_type(self):
        ...
        self.assertEqual(
            called_transaction_data.amount, test_amount, "Сумма должна соответствовать введенной"
        )
    ...
    def test_modal_initialization_with_form_fields(self):
        ...
        self.assertIsInstance(
            self.modal.amount_field, ft.TextField, "Поле суммы должно быть TextField"
        )
        self.assertEqual(
            self.modal.amount_field.label, "Сумма", "Поле суммы должно иметь правильный label"
        )
        self.assertEqual(
            self.modal.amount_field.suffix, "₽", "Поле суммы должно иметь суффикс валюты"
        )
        self.assertEqual(
            self.modal.amount_field.keyboard_type,
            ft.KeyboardType.NUMBER,
            "Поле суммы должно иметь числовую клавиатуру",
        )

        # 2. Проверяем dropdown категорий
        self.assertIsNotNone(self.modal.category_dropdown, "Dropdown категорий должен быть создан")
        self.assertIsInstance(
            self.modal.category_dropdown, ft.Dropdown, "Категории должны быть в Dropdown"
        )
        self.assertEqual(
            self.modal.category_dropdown.label,
            "Категория",
            "Dropdown категорий должен иметь правильный label",
        )
        self.assertIsInstance(
            self.modal.category_dropdown.options, list, "Dropdown должен иметь список опций"
        )

        # 3. Проверяем поле описания
        self.assertIsNotNone(self.modal.description_field, "Поле описания должно быть создано")
        self.assertIsInstance(
            self.modal.description_field, ft.TextField, "Поле описания должно быть TextField"
        )
        self.assertEqual(
            self.modal.description_field.label,
            "Описание (необязательно)",
            "Поле описания должно иметь правильный label",
        )
        self.assertTrue(
            self.modal.description_field.multiline, "Поле описания должно быть многострочным"
        )
        self.assertEqual(
            self.modal.description_field.max_lines,
            3,
            "Поле описания должно иметь максимум 3 строки",
        )
        ...
        self.assertEqual(
            self.modal.date_button.icon,
            ft.Icons.CALENDAR_TODAY,
            "Кнопка даты должна иметь иконку календаря",
        )

        # 5. Проверяем предустановку выбранной даты
        expected_date_text = test_date.strftime("%d.%m.%Y")
        self.assertEqual(
            self.modal.date_button.text,
            expected_date_text,
            f"Кнопка даты должна показывать предустановленную дату: {expected_date_text}",
        )
        self.assertEqual(
            self.modal.current_date, test_date, "Внутренняя дата должна быть установлена правильно"
        )

        # 6. Проверяем режим создания новой транзакции
        self.assertEqual(
            self.modal.dialog.title.value,
            "Новая транзакция",
        ...
        self.assertIsInstance(
            self.modal.type_radio, ft.RadioGroup, "Селектор типа должен быть RadioGroup"
        )
        self.assertEqual(
            self.modal.type_radio.value,
        ...
        self.assertEqual(
            len(self.modal.dialog.actions), 2, "Должно быть 2 кнопки: Отмена и Сохранить"
        )

        # Проверяем текст кнопок
        action_texts = [
            action.text for action in self.modal.dialog.actions if hasattr(action, "text")
        ]
        ...
        self.assertEqual(
            self.modal.error_text.color, ft.Colors.ERROR, "Поле ошибок должно иметь цвет ошибки"
        )
    ...
    def test_invalid_amount_format_validation(self):
        ...
        for invalid_amount in invalid_amounts:
            with self.subTest(amount=invalid_amount):
                ...
                self.assertTrue(
                    any(msg in self.modal.amount_field.error_text for msg in expected_messages),
    ...
    def test_save_button_state_with_validation_errors(self):
        ...
        self.assertFalse(
            hasattr(save_button, "disabled") and save_button.disabled,
    ...
    def test_description_length_validation(self):
        ...
        # Assert - проверяем ошибку валидации
        self.assertEqual(
            self.modal.description_field.error_text, "Описание не может превышать 500 символов"
        )
    ...
    def test_date_validation_edge_cases(self):
        ...
        self.assertEqual(
            self.modal.date_picker.first_date,
            first_date,
            "DatePicker должен иметь правильную минимальную дату",
        )
        self.assertEqual(
            self.modal.date_picker.last_date,
            last_date,
    ...
    def test_date_picker_constraints(self):
        ...
        self.assertGreaterEqual(
            current_date, min_date, "Текущая дата должна быть не раньше минимальной"
        )
        self.assertLessEqual(
            current_date, max_date, "Текущая дата должна быть не позже максимальной"
        )

        # Проверяем, что дата по умолчанию (сегодня) валидна
        today = datetime.date.today()
        self.assertGreaterEqual(
            today, min_date, "Сегодняшняя дата должна быть в допустимом диапазоне"
        )
    ...
    def test_validation_error_clearing(self):
        ...
        self.assertIsNotNone(
            self.modal.amount_field.error_text, "Ошибка суммы должна быть установлена"
        )
        self.assertIsNotNone(
            self.modal.category_dropdown.error_text, "Ошибка категории должна быть установлена"
        )
        ...
        self.assertTrue(
            self.modal.amount_field.error_text is None or self.modal.amount_field.error_text == "",
        ...
        # В Flet error_text может быть None или пустой строкой после очистки
        self.assertTrue(
            self.modal.category_dropdown.error_text is None
            or self.modal.category_dropdown.error_text == "",
    ...
    def test_cancel_button_click(self):
        ...
        self.assertEqual(
            self.modal.amount_field.value, "150.75", "Данные формы должны сохраниться при отмене"
        )
        self.assertEqual(
            self.modal.category_dropdown.value,
            self.cat_id_1,
        ...
        self.assertEqual(
            self.modal.description_field.value,
    ...
    def test_form_reset_after_cancel_and_reopen(self):
        ...
        self.assertEqual(
            self.modal.current_date, second_date, "Внутренняя дата должна быть обновлена"
        )
        ...
        self.assertEqual(self.modal.error_text.value, "", "Общие ошибки должны быть сброшены")
        ...
        self.assertGreater(
            len(self.modal.category_dropdown.options),
            0,
        ...
        self.assertTrue(
            self.modal.category_dropdown.value is None or self.modal.category_dropdown.value == "",
    ...
    def test_cancel_preserves_transaction_list_state(self):
        ...
        self.assertEqual(
            self.on_save.call_count, initial_call_count, "Callback не должен быть вызван при отмене"
        )
    ...
    def test_cancel_with_validation_errors(self):
        ...
        self.assertIsNotNone(
            self.modal.amount_field.error_text, "Должна быть ошибка валидации для пустой суммы"
        )
        self.assertIsNotNone(
            self.modal.category_dropdown.error_text,
        ...
        self.assertIsNotNone(
            self.modal.amount_field.error_text, "Ошибки валидации должны сохраниться при отмене"
        )
        self.assertIsNotNone(
            self.modal.category_dropdown.error_text,
    ...
    def test_multiple_cancel_operations(self):
        ...
        for i, scenario in enumerate(test_scenarios):
            with self.subTest(scenario=i + 1):
                ...
                (
                    self.page.close.assert_called(),
                    f"Модальное окно должно закрыться в сценарии {i + 1}",
                )
        ...
        self.page.open.assert_called()
        self.assertEqual(
            self.modal.current_date, final_date, "Дата должна устанавливаться корректно"
        )
        self.assertEqual(
            self.modal.amount_field.value, "", "Форма должна быть очищена при новом открытии"
        )
...
class TestTransactionModalProperties:
    ...
    @given(
        amount_value=st.one_of(
            ...
            st.text(alphabet="abcdefghijklmnopqrstuvwxyz!@#$%^&*()").filter(
                lambda x: len(x) > 0
            ),  # Некорректный формат
            st.floats(min_value=-1000, max_value=0, allow_nan=False, allow_infinity=False).map(
                str
            ),  # Отрицательные/нулевые
            st.decimals(min_value=Decimal("0.01"), max_value=Decimal("999999.99")).map(
                str
            ),  # Валидные суммы
        ),
    ...
    def test_property_6_form_validation_state(
        self, amount_value, category_value, description_value
    ):
        """
        ...
        **Validates: Requirements 5.2, 6.5**
        ...
        with patch(
            ...
            if not amount_valid:
                ...
                assert (
                    modal.amount_field.error_text is None or modal.amount_field.error_text == ""
            ...
            if not category_valid:
                ...
                assert (
                    modal.category_dropdown.error_text is None
                    or modal.category_dropdown.error_text == ""
    ...
    def test_property_7_transaction_creation_round_trip(
        self, amount, transaction_type, description, transaction_date
    ):
        """
        ...
        **Validates: Requirements 5.3, 5.5**
        ...
        with patch(
            ...
            if transaction_type == TransactionType.EXPENSE:
                modal.category_dropdown.options = [
                    ft.dropdown.Option(key=expense_category_id, text="Test Expense Category")
                ]
            else:
                modal.category_dropdown.options = [
                    ft.dropdown.Option(key=income_category_id, text="Test Income Category")
                ]
            ...
            assert called_transaction_data.category_id is not None, (
                "ID категории не должен быть None"
            )
            ...
            assert called_transaction_data.type in [
                TransactionType.INCOME,
                TransactionType.EXPENSE,
            ...
            assert (
                called_transaction_data.transaction_date == expected_transaction.transaction_date
...
class TestTransactionModalCancelProperties:
    ...
    @settings(max_examples=50, deadline=None)
    def test_property_8_modal_closure_behavior(
        self, closure_method, amount_value, description_value, transaction_date
    ):
        """
        ...
        **Validates: Requirements 5.4, 7.1**
        ...
        with patch(
            ...
            modal.description_field.value = (
                description_value if description_value is not None else ""
            )
            ...
            try:
                ...
                pytest.fail(
                    f"Повторное открытие должно работать после закрытия через {closure_method}: {e}"
                )
    ...
    @settings(max_examples=50, deadline=None)
    def test_property_9_data_persistence_on_cancel(
        self, initial_amount, initial_description, transaction_date, cancel_method
    ):
        """
        ...
        **Validates: Requirements 7.4**
        ...
        with patch(
            ...
            modal.category_dropdown.value = test_category_id
            modal.description_field.value = (
                initial_description if initial_description is not None else ""
            )

            # Проверяем, что данные заполнены (потенциально готовы к сохранению)
            (
                (modal.amount_field.value and modal.amount_field.value.strip())
                or (modal.description_field.value and modal.description_field.value.strip())
                or modal.category_dropdown.value is not None
            )
    ...
    @settings(max_examples=50, deadline=None)
    def test_property_10_form_reset_on_cancel(
        self, form_data_before_cancel, reopen_date, cancel_method
    ):
        """
        ...
        **Validates: Requirements 7.5**
        ...
        with patch(
            ...
            modal.amount_field.value = (
                form_data_before_cancel["amount"]
                if form_data_before_cancel["amount"] is not None
                else ""
            )
            modal.category_dropdown.value = selected_category_id
            modal.description_field.value = (
                form_data_before_cancel["description"]
                if form_data_before_cancel["description"] is not None
                else ""
            )
            ...
            mock_on_save.assert_called_once(), "Форма должна работать нормально после сброса"
...
class TestTransactionModalEditMode(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.modal = TransactionModal(
            session=self.session, on_save=self.on_save, on_update=self.on_update
        )
        self.page = MagicMock()
        self.page.open = Mock()
        self.page.close = Mock()
    ...
    def test_create_mode_title_and_behavior(self):
        ...
        self.assertEqual(
            self.modal.date_button.text, "15.12.2024", "Дата должна быть установлена правильно"
        )
    ...
    def test_edit_mode_comprehensive_prefill(self):
        ...
        self.assertEqual(
            self.modal.editing_transaction,
            edit_transaction,
        ...
        self.assertEqual(
            self.modal.amount_field.value, "350.25", "Поле суммы должно быть предзаполнено"
        )

        # 3. Проверяем предзаполнение описания
        self.assertEqual(
            self.modal.description_field.value,
            "Comprehensive test transaction",
    ...
    def test_on_update_callback_invocation(self):
        ...
        self.assertEqual(
            transaction_id, self.transaction_id, "ID транзакции должен быть передан правильно"
        )

        # 2. Проверяем тип объекта обновления
        self.assertIsInstance(
            transaction_update, TransactionUpdate, "Должен быть передан объект TransactionUpdate"
        )

        # 3. Проверяем данные обновления
        self.assertEqual(
            transaction_update.amount, Decimal("275.80"), "Сумма должна быть обновлена правильно"
        )
        self.assertEqual(
        ...
        self.assertEqual(
            transaction_update.category_id, self.cat_id_1, "ID категории должен быть правильным"
        )
        self.assertEqual(
            transaction_update.transaction_date,
            new_date,
