...
class TestPendingPaymentProperties:
    ...
    def test_property_30_category_validation(self, amount, description, priority, planned_date):
        """
        ...
        with get_test_session() as session:
            ...
            expense_parent_category = CategoryDB(
                name=f"ExpenseParent_{description[:10]}",
                type=TransactionType.EXPENSE,
                is_system=False,
            )
            session.add(expense_parent_category)
            session.flush()

            expense_leaf_category = CategoryDB(
                name=f"ExpenseLeaf_{description[:10]}",
                type=TransactionType.EXPENSE,
                parent_id=expense_parent_category.id,
                is_system=False,
            )
            income_category = CategoryDB(
                name=f"Income_{description[:10]}", type=TransactionType.INCOME, is_system=False
            )
            session.add(expense_leaf_category)
            session.add(income_category)
            session.commit()

            payment_data = PendingPaymentCreate(
                amount=amount,
                category_id=expense_leaf_category.id,
            ...
            assert payment.category_id == expense_leaf_category.id
            assert payment.status == PendingPaymentStatus.ACTIVE

            invalid_parent_payment_data = PendingPaymentCreate(
                amount=amount,
                category_id=expense_parent_category.id,
                description=description,
                priority=priority,
                planned_date=planned_date,
            )
            ...
            with pytest.raises(ValueError, match="конечные категории"):
                create_pending_payment(session, invalid_parent_payment_data)
    ...
    @given(amount=amounts, description=descriptions, executed_amount=amounts)
    @settings(max_examples=100, deadline=None)
    def test_property_31_execute_payment(self, amount, description, executed_amount):
        """
        ...
        with get_test_session() as session:
            ...
            expense_category = CategoryDB(
                name=f"Expense_{description[:10]}", type=TransactionType.EXPENSE, is_system=False
            )
            ...
            execute_data = PendingPaymentExecute(
                executed_date=date.today(), executed_amount=executed_amount
            )

            transaction, updated_payment = execute_pending_payment(
                session, payment_id, execute_data
            )
    ...
    @settings(max_examples=100, deadline=None)
    def test_property_32_cancel_payment(self, amount, description, cancel_reason):
        """
    ...
    @given(payments_count=st.integers(min_value=2, max_value=10))
    @settings(max_examples=50, deadline=None)
    def test_property_33_payment_sorting(self, payments_count):
        """
        ...
        with get_test_session() as session:
            # Создаём тестовую категорию
            expense_category = CategoryDB(
                name="Expense_Sort", type=TransactionType.EXPENSE, is_system=False
            )
            session.add(expense_category)
            session.commit()
    ...
    @given(payments_count=st.integers(min_value=1, max_value=20))
    @settings(max_examples=50, deadline=None)
    def test_property_34_payment_statistics(self, payments_count):
        """
        ...
        with get_test_session() as session:
            # Создаём тестовую категорию
            expense_category = CategoryDB(
                name="Expense_Stats", type=TransactionType.EXPENSE, is_system=False
            )
            session.add(expense_category)
            session.commit()
    ...
    def test_property_1_create_payment_saves_to_db(
        self, amount, description, priority, planned_date
    ):
        """
        **Feature: pending-payment-add-button, Property 1: Создание платежа сохраняет в БД**
        **Validates: Requirements 1.3**
        ...
        with get_test_session() as session:
            ...
            saved_payment = session.query(PendingPaymentDB).filter_by(id=created_payment.id).first()
        ...
        with get_test_session() as session:
            ...
            for payment in payments_list:
                if (
                    ...
                    and payment.description == description.strip()
                    and payment.priority == priority
                ):
                    created_payment_found = True
                    break

            assert created_payment_found, (
                "Созданный платёж должен быть в списке обновлённых платежей"
            )
    ...
    def test_property_4_payment_with_date_updates_calendar(
        ...
        **Validates: Requirements 5.3**
        ...
        with get_test_session() as session:
            ...
            created_payment = (
                session.query(PendingPaymentDB).filter_by(description=description.strip()).first()
            )
    ...
    @settings(max_examples=100, deadline=None)
    def test_property_5_cascade_updates(self, amount, description, priority, planned_date):
        """
        **Feature: pending-payment-add-button, Property 5: Каскадное обновление компонентов**
        ...
        with get_test_session() as session:
            ...
            update_calls = [
                ...
                in [
                    ...
                    "update_transactions",
                    "update_planned_occurrences",
                    "update_pending_payments",
                ]
            ]
            show_message_calls = [
                i for i, call in enumerate(all_calls) if call[0] == "show_message"
            ]
    ...
    @given(
        ...
        invalid_description=st.one_of(st.just(""), st.just("   "), st.just("\t"), st.just("\n")),
    )
    @settings(max_examples=100, deadline=None)
    def test_property_6_validation_rejects_invalid_data(self, invalid_amount, invalid_description):
        """
        **Feature: pending-payment-add-button, Property 6: Валидация пустых полей**
        **Validates: Requirements 6.1**
        ...
        with get_test_session() as session:
            # Создаём тестовую категорию EXPENSE
            expense_category = CategoryDB(
                name="Expense_Validation", type=TransactionType.EXPENSE, is_system=False
            )
            session.add(expense_category)
            session.commit()
            ...
            assert any(error["loc"] == ("amount",) for error in errors), (
                f"Должна быть ошибка валидации для поля 'amount', но получены ошибки: {errors}"
            )
            ...
            assert len(error_messages) > 0, "Должно быть сообщение об ошибке для поля 'amount'"
            ...
            assert any(error["loc"] == ("description",) for error in errors), (
                f"Должна быть ошибка валидации для поля 'description', но получены ошибки: {errors}"
            )
            ...
            assert len(error_messages) > 0, "Должно быть сообщение об ошибке для поля 'description'"

            # Проверяем, что сообщение содержит информацию о проблеме
            combined_message = " ".join(error_messages).lower()
            assert (
                "пустым" in combined_message
                or "empty" in combined_message
                or "at least" in combined_message
                or "минимум" in combined_message
                or "length" in combined_message
