...
class TestPlanFactView(ViewTestBase):
    ...
    def setUp(self):
        ...
        self.mock_get_db = self.add_patcher(
            "finance_tracker.views.plan_fact_view.get_db", return_value=self.mock_db_cm
        )
        ...
        self.mock_get_all_categories = self.add_patcher(
            "finance_tracker.views.plan_fact_view.get_all_categories", return_value=[]
        )
    ...
    def test_load_data_on_mount(self):
        ...
        self.assert_service_called_once(self.mock_get_all_categories, self.mock_session)
    ...
    def test_change_period(self):
        ...
        self.assert_service_called_once(
            self.mock_get_plan_fact_analysis, self.mock_session, new_start, new_end, None
        )
    ...
    def test_filter_by_category(self):
        ...
        test_categories = [
            create_test_category(id=category_id, name="Продукты"),
            create_test_category(id=str(uuid4()), name="Транспорт"),
        ]
        ...
        self.view.category_dropdown.value = category_id
        self.view._on_category_change(None)

        # Проверяем, что фильтр установлен
        self.assertEqual(self.view.selected_category_id, category_id)
    ...
    def test_filter_by_category_all(self):
        ...
        self.view.selected_category_id = str(uuid4())
    ...
    def test_load_categories_for_filter(self):
        ...
        # Создаем тестовые категории
        test_categories = [
            create_test_category(id=str(uuid4()), name="Продукты"),
            create_test_category(id=str(uuid4()), name="Транспорт"),
            create_test_category(id=str(uuid4()), name="Развлечения"),
        ]
        self.mock_get_all_categories.return_value = test_categories
    ...
    def test_category_dropdown_uses_parent_child_label_for_child(self):
        parent_id = str(uuid4())
        child_id = str(uuid4())
        categories = [
            create_test_category(id=parent_id, name="Еда", parent_id=None),
            create_test_category(id=child_id, name="Кафе", parent_id=parent_id),
        ]
        self.mock_get_all_categories.return_value = categories

        self.view._load_categories()

        option_by_key = {option.key: option.text for option in self.view.category_dropdown.options}
        self.assertEqual(option_by_key[child_id], "Еда / Кафе")

    def test_uuid_category_filter_does_not_cast_to_int(self):
        category_id = str(uuid4())
        categories = [create_test_category(id=category_id, name="Продукты")]
        self.mock_get_all_categories.return_value = categories
        self.view._load_categories()
        self.mock_get_plan_fact_analysis.reset_mock()
    ...
    def test_parent_category_filter_includes_children(self):
        ...
        categories = [
            create_test_category(id=parent_id, name="Еда", parent_id=None),
            create_test_category(id=child_id, name="Кафе", parent_id=parent_id),
            create_test_category(id=other_id, name="Транспорт", parent_id=None),
        ]
        self.mock_get_all_categories.return_value = categories
        self.view._load_categories()
        ...
        analysis["occurrences"] = [
            {
                ...
                "amount_deviation": Decimal("-50.00"),
                "date_deviation": 0,
                "status": "executed",
            },
            {
        ...
        analysis["total_occurrences"] = 3
        analysis["executed_count"] = 2
        analysis["pending_count"] = 1
        analysis["skipped_count"] = 0
        analysis["avg_amount_deviation"] = Decimal("-25.00")
        analysis["avg_date_deviation_days"] = 0.5
        analysis["on_time_percentage"] = 50.0
        analysis["skipped_percentage"] = 0.0
        self.mock_get_plan_fact_analysis.return_value = analysis
        self.mock_get_plan_fact_analysis.reset_mock()

        self.view.category_dropdown.value = parent_id
        self.view._on_category_change(None)
        ...
        self.assertEqual(len(self.view.data_table.rows), 2)
        total_text = self.view.stat_total.content.controls[1].controls[1].value
        self.assertEqual(total_text, "2")
