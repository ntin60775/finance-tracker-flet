...
class TestCalendarLegendIntegration(unittest.TestCase):
    ...
    def test_calendar_legend_integration_with_home_view(self):
        ...
        with patch("finance_tracker.database.get_db_session") as mock_get_db:
            ...
            main_row = cast(ft.Row, home_view.controls[0])
            ...
            center_column = cast(ft.Column, columns[2])
    ...
    def test_indicator_consistency_between_calendar_and_legend(self):
        ...
        with patch("finance_tracker.database.get_db_session") as mock_get_db:
            ...
            assert income_indicator is not None
            assert expense_indicator is not None

            # Проверяем цвета (должны соответствовать календарю)
            self.assertEqual(
                cast(ft.Container, income_indicator.visual_element).bgcolor,
                ft.Colors.GREEN,
                "Цвет индикатора доходов должен быть зелёным",
            )
            self.assertEqual(
                cast(ft.Container, expense_indicator.visual_element).bgcolor,
                ft.Colors.RED,
            ...
            assert planned_indicator is not None
            assert pending_indicator is not None
            assert loan_indicator is not None

            # Проверяем символы (должны соответствовать календарю)
            self.assertEqual(
                cast(ft.Text, planned_indicator.visual_element).value,
                "◆",
                "Символ плановых транзакций должен быть ◆",
            )
            self.assertEqual(
                cast(ft.Text, pending_indicator.visual_element).value,
                "📋",
                "Символ отложенных платежей должен быть 📋",
            )
            self.assertEqual(
                cast(ft.Text, loan_indicator.visual_element).value,
                "💳",
            ...
            assert cash_gap_indicator is not None
            assert overdue_indicator is not None

            # Проверяем цвета фона (должны соответствовать календарю)
            self.assertEqual(
                cast(ft.Container, cash_gap_indicator.visual_element).bgcolor,
                ft.Colors.AMBER_100,
                "Цвет индикатора кассовых разрывов должен быть жёлтым",
            )
            self.assertEqual(
                cast(ft.Container, overdue_indicator.visual_element).bgcolor,
                ft.Colors.RED_100,
    ...
    def test_legend_updates_with_calendar_data(self):
        ...
        with patch("finance_tracker.database.get_db_session") as mock_get_db:
            ...
            calendar_widget.set_transactions(
                cast(list[Any], mock_transactions), cast(list[Any], mock_occurrences)
            )
            ...
            for i in range(5):
                ...
                calendar_widget.set_transactions(cast(list[Any], updated_transactions), [])
    ...
    def test_property_13_responsive_stability(self, calendar_width):
        ...
        **Validates: Requirements 5.5**
        ...
        with patch("finance_tracker.database.get_db_session") as mock_get_db:
            ...
            # 5. При компактном режиме кнопка details не используется
            if not actual_full_mode:
                ...
                assert len(details_buttons) == 0, (
                    "В компактном режиме кнопка details не должна отображаться"
                )

            assert not hasattr(legend, "modal_manager")
    ...
    def test_integration_error_handling(self):
        ...
        with patch("finance_tracker.database.get_db_session") as mock_get_db:
            ...
            try:
                home_view = HomeView(self.mock_page, cast(Any, None))
